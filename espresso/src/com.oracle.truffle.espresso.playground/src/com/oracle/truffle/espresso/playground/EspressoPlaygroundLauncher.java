...
package com.oracle.truffle.espresso.playground;
...
public final class EspressoPlaygroundLauncher extends AbstractLanguageLauncher {
    ...
    public static void main(String[] args) {
        new EspressoPlaygroundLauncher().launch(args);
    }
    ...
    private void spawnContext(Builder contextBuilder) {
        try (Context context = contextBuilder.build()) {
            long startms = System.currentTimeMillis();
            context.initialize("java");
            long endms = System.currentTimeMillis();
            System.out.println("context init: " + (endms - startms) + "ms");

            context.enter();
            EspressoContext ctx = EspressoLanguage.getCurrentContext();
            //ctx.getCache();
            //ctx.getRegistries();
            context.leave();
        }
    ...
    protected void launch(Builder contextBuilder) {
        ...
        Engine engine = Engine.newBuilder().in(System.in).out(System.out).err(System.err).allowExperimentalOptions(true).build();
        contextBuilder.allowCreateThread(true).engine(engine);

        for (int i = 0; i < 10; i++)
            spawnContext(contextBuilder);
